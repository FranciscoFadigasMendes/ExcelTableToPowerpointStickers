...
PPT_PATH = r"C:\LOTO PLACARDS FC08\PLC90\02. Energy Point Tags\PLC90_LOTO_EnergyTags_241_Template_Italian.pptx"

SHEET_NAME = "EnergyTags"  # Adjust sheet name
TOTAL_TAGS = 241
TAGS_PER_SLIDE = 10
START_ROW = 2  # Excel row Q2 = first tag
EXCEL_COL = "Q"  # Column containing the tag values
...
def main():
    print("Starting PLC90 Energy Tag update...")
    ...
    for tag_index in range(1, TOTAL_TAGS + 1):
        ...
        if slide_index > pres.Slides.Count:
            print(f"Slide {slide_index} missing — {pos_print}")
            continue
        ...
        if shp is None:
            print(f"Missing shape \"{shape_name}\" {pos_print}")
            continue
    ...
    print("DONE! 241 Energy Tags Updated Successfully!")
